...
mod tests {
    ...
    fn test_parse_request_with_params() {
        let input = r#"{"jsonrpc":"2.0","method":"tools/call","params":{"name":"init_project","arguments":{"name":"test"}},"id":2}"#;
        let req: JsonRpcRequest = serde_json::from_str(input).unwrap();
    ...
    fn test_serialize_tool_definition() {
        let tool = ToolDefinition {
            ...
            description: "Create a new this-rs project".to_string(),
        ...
        assert!(json.contains("init_project"));
    }

    #[test]
    fn test_parse_tool_call_params() {
        let input = r#"{"name":"init_project","arguments":{"name":"my-api","port":4000}}"#;
        let params: ToolCallParams = serde_json::from_str(input).unwrap();
    ...
    fn test_tool_call_result_success() {
        let result = ToolCallResult::success(r#"{"status":"ok"}"#.to_string());
